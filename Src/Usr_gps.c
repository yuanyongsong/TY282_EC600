...
//接收基站数据:
//+QENG: 1,1,617,-93,48,66,498,460,00,248a,e45,2,52,-93,44,57,-1000,460,00,10d5,105d,3,49,-100,32,15,447,460,00,248a,f17,
//4,629,-101,25,36,500,460,00,248a,e57,5,613,-103,21,25,473,460,00,248a,f0c,6,622,-105,57,-1,-1,460,00,248a,fde
//这里一共有6个基站数据，在没有搜索到基站时基站数据为x，已搜索到的基站优先填充。例如：
//+QENG: 1,1,617,-92,48,71,503,460,00,248a,e45,2,52,-95,44,52,-1000,460,00,10d5,105d,3,x,x,x,x,x,x,x,x,x,4,x,x,x,x,x,x,x,x,x,5,x,x,x,x,x,x,x,x,x,6,x,x,x,x,x,x,x,x,x
//每个基站格式依次为：基站ID,bcch,dbm,bsic,c1,c2,mcc,mnc,lac,cellid
void BLS_DataProcess(char *pSrc)
{
	char *ptem = NULL, *p1 = NULL, *p2 = NULL;
	unsigned char i, j = 0;
	u32 u32_Temp;
	u8 error = 0, LBS_Num_temp;
	memset(LBS_Piont, '\0', 68);
	ptem = pSrc;
	i = 3;
	while (i--)
	{
		if ((ptem = strstr(ptem, "+QENG:")) != NULL)
			ptem++;
		else
			break;
	}
	...
	{
		LBS_Num = 0;
		if ((p1 = strstr(ptem, ",x,x,")) == NULL)
			LBS_Num = 6;
		else
			LBS_Num = *(--p1) - '1';
		LBS_Piont[j] = LBS_Num;
		LBS_Num_temp = LBS_Num;
		j++;
		ptem = strstr(ptem, ",1");
		ptem++; //指向第一个基站的序号
		...
		while (LBS_Num_temp--)
		{
			...
			if (i == 0)
				break;
			if (!Flag.HaveGetMccMnc)
			{
				memset(MccMnc, '\0', 7);
				strncpy(MccMnc, ptem, i);
			}
			u32_Temp = (u32)Ascii2BCD_u16(ptem, i); //MCC
			if (u32_Temp == 0)
				break;
			LBS_Piont[j++] = (u8)(u32_Temp >> 8);
			LBS_Piont[j++] = (u8)(u32_Temp);
			...
			if (!Flag.HaveGetMccMnc)
			{
				strncat(MccMnc, ptem, i);
				Flag.HaveGetMccMnc = 1;
			}
			u32_Temp = (u32)Ascii2BCD_u16(ptem, i); //MNC
			if (u32_Temp == 0xFF)
				break;
			LBS_Piont[j++] = (u8)(u32_Temp >> 8);
			LBS_Piont[j++] = (u8)(u32_Temp);
			...
			if (i == 0)
				break;
			u32_Temp = Ascii2Hex(ptem, i); //CID
			if (u32_Temp == 0)
				break;
			LBS_Piont[j++] = (u8)(u32_Temp >> 24);
			LBS_Piont[j++] = (u8)(u32_Temp >> 16);
			LBS_Piont[j++] = (u8)(u32_Temp >> 8);
			LBS_Piont[j++] = (u8)(u32_Temp);
			...
			if (i == 0)
				break;
			ptem++;									//过滤掉负号
			u32_Temp = (u32)Ascii2BCD_u16(ptem, i); //信号强度
			if (u32_Temp == 0)
				break;
			LBS_Piont[j++] = (u8)(u32_Temp);
